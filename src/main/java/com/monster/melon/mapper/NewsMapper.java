...
public interface NewsMapper {
    ...
    @Select("select a.*,count(b.id) as comment_num from news as a left join comment as b on b.news_id = a.id where admin_opt = 0 group by id limit #{start},#{end}")
    ...
    @Select("select a.*,count(b.content) as like_count from news as a left join comment as b on a.id = b.news_id where a.id = #{id};")
